...
// Extract components using string manipulation to be safe
let tabsStart = content.indexOf('<div className="flex bg-mono-100 dark:bg-white/5 p-1 rounded-md mb-6 relative">');
let formStart = content.indexOf('<form onSubmit={handleAuth}');
let dividerStart = content.indexOf('<div className="mt-4">');
let containerEnd = content.indexOf('</div>', content.lastIndexOf('</>')) // rough approximation
...
if (tabsStart > 0 && formStart > 0 && dividerStart > 0) {
    ...
    // Now extract the divider contents
    ...
    let guestStart = content.indexOf('<button', googleStart + 100);
    // actually, let's find guest button by onClick={handleGuestLogin}
...
`;
    ...
    console.log("Could not find required blocks", {tabsStart, formStart, dividerStart});
}
