...
// 2. Add Conditional Render Start before TABS
let tabsString = '{/* TABS */}\n                                <div className="flex bg-mono-100 dark:bg-white/5 p-1 rounded-md mb-6 relative" dir="rtl">';

let startReplacement = `                                {!showEmailForm ? (
                                    <div className="w-full flex flex-col mt-2 space-y-3">
                                        ...
                                        <div className="flex items-center justify-center gap-4 text-[9px] font-black font-rabar text-mono-400 dark:text-white/30 uppercase mt-2">
                                ...
                                {/* TABS */}
                                <div className="flex bg-mono-100 dark:bg-white/5 p-1 rounded-md mb-6 relative" dir="rtl">`;
content = content.replace(tabsString, startReplacement);


// 3. Remove old social divider and buttons, replace with closing tag
let oldSocialStart = content.indexOf('{/* DIVIDER for Social */}');
...
let oldSocialEnd = content.indexOf(oldSocialEndStr) + oldSocialEndStr.length;
...
if (oldSocialStart > 0 && oldSocialEnd > oldSocialStart) {
    ...
    console.log("Could not find old social block boundaries!");
}

fs.writeFileSync(file, content, 'utf8');
console.log("Button-first flow applied safely!");
