...
export default function BottomNav({ currentView, setCurrentView, onSettingsToggle, onTabClickSound, chatBadgeCount = 0, hasSilentGlobal = false, pendingFriendsCount = 0, hasUnclaimedRewards = false }) {
  ...
  return (
    <LayoutGroup>
      ...
      <nav className="sticky bottom-0 w-full z-40 min-h-[80px] pb-[max(1rem,env(safe-area-inset-bottom))] pt-2 bg-mono-white dark:bg-black border-t border-mono-200 dark:border-white/5 flex justify-evenly items-center px-2 leading-none" dir="rtl">
        {tabs.map((tab) => {
