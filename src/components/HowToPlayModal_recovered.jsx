...
export default function HowToPlayModal({ isOpen, onClose, initialMode = 'classic', showTabs = true }) {
  ...
  const renderExplanations = (correctJsx, wrongPosJsx, absentJsx) => (
    <div className="space-y-3 bg-white p-3.5 rounded-xl border border-[#cbd5e1] shadow-sm mt-4">
      <div className="flex gap-2.5 items-start">
        <div className="w-4 h-4 mt-0.5 rounded-sm bg-[#22c55e] shrink-0 border border-[#166534] border-b-[2px] border-b-[#14532d] shadow-[inset_0_1.5px_0_rgba(255,255,255,0.25)]" />
      ...
      <div className="flex gap-2.5 items-start">
        <div className="w-4 h-4 mt-0.5 rounded-sm bg-[#f59e0b] shrink-0 border border-[#b45309] border-b-[2px] border-b-[#78350f] shadow-[inset_0_1.5px_0_rgba(255,255,255,0.3)]" />
        <p className="text-[11px] font-bold text-[#727888] leading-relaxed flex-1">
          {wrongPosJsx}
        </p>
      </div>
      ...
      <div className="flex gap-2.5 items-start">
        <div className="w-4 h-4 mt-0.5 rounded-sm bg-slate-400 shrink-0 border border-slate-500 border-b-[2px] border-b-slate-600 shadow-[inset_0_1.5px_0_rgba(255,255,255,0.15)]" />
        <p className="text-[11px] font-bold text-[#727888] leading-relaxed flex-1">
          {absentJsx}
        </p>
      </div>
  ...
  const renderClassicTutorial = () => (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="w-full flex justify-center px-2 sm:px-6 my-2 shrink-0">
        <GameResultRenderer 
          text={`پەیڤۆک\nپ🟩 س د ژ ف\nێ🟨 م🟨 و ی ژ\nر⬛ ا س ت ی`} 
        />
      </div>
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-3">
        ...
        <ul className="space-y-2">
          <li className="flex gap-2 text-[12px] font-bold text-[#4a5568] leading-relaxed">
            <span className="text-[#a0a7b4]">•</span>
            <span>ئارمانجا یاریێ ئەوە تو پەیڤا ڤەشارتی د {toKuDigits(6)} بزاڤان دا ببینی.</span>
          </li>
          <li className="flex gap-2 text-[12px] font-bold text-[#4a5568] leading-relaxed">
            <span className="text-[#a0a7b4]">•</span>
            <span>مۆدێ (مامک) ژ وان پەیڤان پێکدهێت ئەڤێن کو ژ {toKuDigits(2)} تا ∞ پیتان پێکدهێن.</span>
          </li>
        </ul>
        <p className="text-[12px] font-bold text-[#4a5568] leading-relaxed">
                      ...
                      <div className="flex flex-wrap justify-center gap-2 shrink-0">
                         {gameModes.map(mode => (
                            ...
                            >
                               ...
                               <span className={`relative z-10 ${activeTab === mode.id ? 'drop-shadow-md' : ''}`} style={activeTab === mode.id ? { textShadow: '-1px -1px 0 #115ab5, 1px -1px 0 #115ab5, -1px 1px 0 #115ab5, 1px 1px 0 #115ab5, 0 1px 0 #115ab5' } : { }}>
                                  {mode.title}
                               </span>
                            </button>
                         ))}
