...
BEGIN
  ...
  SELECT 
    ...
    END,
    owned_bundles
  INTO v_curr_balance, v_owned_bundles
  FROM profiles WHERE id = auth.uid();

  -- Handle NULL array
  v_owned_bundles := COALESCE(v_owned_bundles, ARRAY['default']::TEXT[]);
  ...
  IF p_item_id = ANY(v_owned_bundles) THEN
  ...
  SET
    ...
    owned_bundles = v_owned_bundles || p_item_id,
    updated_at = NOW()
  WHERE id = auth.uid();
