...
const sqlFilePath = path.join(__dirname, '../insert_all_words_grouped.sql');
...
function parseAndSync() {
  console.log('Reading insert_all_words_grouped.sql...');
  ...
  // Regex to match: ('word', 'hint', 'category')
  ...
  lines.forEach(line => {
    ...
    if (match) {
      ...
      let hint = match[2].replace(/''/g, "'").trim();
      let category = match[3].replace(/''/g, "'").trim();
  ...
  console.log(`Parsed and deduplicated ${rawCount} unique words from SQL file.`);
