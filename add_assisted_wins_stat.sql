-- 1. Add assisted_wins column to profiles table
ALTER TABLE public.profiles 
  ADD COLUMN IF NOT EXISTS assisted_wins INTEGER DEFAULT 0;

-- 2. Drop previous versions of sync_profile_progression
...
DROP FUNCTION IF EXISTS public.sync_profile_progression(integer,integer,integer,integer,integer,text[],text,integer,boolean,integer,boolean,boolean,boolean,boolean,integer,integer,integer);
DROP FUNCTION IF EXISTS public.sync_profile_progression(integer,integer,integer,integer,integer,text[],text,integer,boolean,integer,boolean,boolean,boolean,boolean,integer,integer,integer,boolean);

-- 3. Recreate sync_profile_progression with p_is_assisted
CREATE OR REPLACE FUNCTION public.sync_profile_progression(
  ...
  p_is_assisted BOOLEAN DEFAULT FALSE
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET 
    ...
    flawless_wins = CASE WHEN p_is_flawless AND p_is_win THEN COALESCE(flawless_wins, 0) + 1 ELSE flawless_wins END,
    assisted_wins = CASE WHEN p_is_assisted AND p_is_win THEN COALESCE(assisted_wins, 0) + 1 ELSE assisted_wins END,
    ...
    mode_play_counts = CASE 
        WHEN NOT p_is_win AND p_mode NOT LIKE '%_loss' AND p_mode NOT LIKE '%_draw' THEN
            jsonb_set(
                jsonb_set(
                ...
                ARRAY[p_mode || '_loss'],
                (COALESCE((mode_play_counts->>(p_mode || '_loss'))::int, 0) + 1)::text::jsonb
            )
        ELSE
            jsonb_set(
                COALESCE(mode_play_counts, '{"classic":0, "battle":0, "mamak":0, "hard_words":0, "word_fever":0, "secret_word":0}'::jsonb),
  ...
  -- 6. Check Level Up
  IF v_new_level > (SELECT level FROM public.profiles WHERE id = v_user_id) THEN
    -- Optionally handle rank up logic here if needed inside the DB
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'streak', v_new_streak,
    'max_streak', v_max_streak,
    'flawless_wins_count', (SELECT flawless_wins FROM public.profiles WHERE id = v_user_id),
    'assisted_wins_count', (SELECT assisted_wins FROM public.profiles WHERE id = v_user_id)
  );
END;
$$;
