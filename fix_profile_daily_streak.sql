...
BEGIN
  ...
  -- 2. Daily Streak Snapchat Logic
  SELECT last_streak_at, daily_streak INTO v_last_streak_at, v_daily_streak FROM public.profiles WHERE id = v_user_id;
  ...
  -- 4. Update XP and calculate NEW level server-side
  v_new_xp := COALESCE(v_new_xp, 0) + p_xp_to_add;
  v_new_level := calculate_rpg_level(v_new_xp);

  -- 5. Calculate streaks and distribution
  IF p_is_win THEN
