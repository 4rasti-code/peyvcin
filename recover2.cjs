...
async function processLineByLine() {
            ...
            if (obj.type === 'PLANNER_RESPONSE' && obj.tool_calls) {
                for (const call of obj.tool_calls) {
                    if (call.name === 'write_to_file' && call.args && call.args.TargetFile && call.args.TargetFile.includes('HowToPlayModal.jsx')) {
                        ...
                        replaces = []; // reset replaces if file is rewritten
    ...
    for (const r of replaces) {
        if (content.includes(r.target)) {
            ...
            console.log('Target not found:', r.target.substring(0, 50));
        }
    }

    fs.writeFileSync('d:/Peyvok_App/src/components/HowToPlayModal_recovered2.jsx', content);
